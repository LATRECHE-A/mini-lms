    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <div class="mb-6 bg-slate-50 rounded-lg border border-slate-200 p-4 overflow-x-auto">
                    <p class="text-xs text-slate-500 mb-3 font-medium">📊 Diagramme</p>
                ...
                @else
                    <p class="text-sm text-slate-400 italic">Aucun contenu.</p>
                @endif

                @if($subchapter->sources && is_array($subchapter->sources) && count($subchapter->sources) > 0)
                <div class="mt-8 pt-6 border-t border-slate-200">
                    <h3 class="text-sm font-semibold text-slate-700 mb-3">📚 Sources</h3>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.subchapters.edit', [$chapter, $subchapter]) }}" class="block text-sm text-brand-600 hover:text-brand-700 font-medium">Modifier le contenu</a>
                    @if($subchapter->quiz)
                    ...
                    @else
                        <a href="{{ route('admin.quizzes.create', $subchapter) }}" class="block text-sm text-brand-600 hover:text-brand-700">Créer un quiz manuellement</a>
                    @endif
                </div>
            </div>
            ...
            @if(!$subchapter->quiz)
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                <h3 class="font-semibold text-slate-900 mb-3">Quiz IA</h3>
                <form method="POST" action="{{ route('admin.ai.generate-quiz', $subchapter) }}">
                    @csrf
                    <button type="submit" class="w-full bg-violet-50 hover:bg-violet-100 border border-violet-200 text-violet-700 text-sm font-medium py-2.5 px-4 rounded-lg transition-colors inline-flex items-center justify-center gap-2"
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('admin.flashcards.subchapter', $subchapter) }}" class="block text-center text-sm text-brand-600 hover:text-brand-700 font-medium">Voir les flashcards →</a>
                    <a href="{{ route('admin.flashcards.study', ['sub_chapter_id' => $subchapter->id]) }}" class="block text-center text-sm text-slate-500 hover:text-slate-700">Étudier →</a>
                </div>
            </div>
