{{-- File: resources/views/admin/flashcards/index.blade.php --}}
@extends('layouts.app')
...
<div class="fade-in">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Créez et testez les flashcards. Les templates sont distribués automatiquement aux apprenants inscrits.</p>
        </div>
        @if($stats['due'] > 0)
        <a href="{{ route('admin.flashcards.study') }}"
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-amber-50 rounded-xl border border-amber-200 p-4 text-center">
            ...
            <p class="text-xs text-amber-600 mt-1">À tester</p>
        </div>
           ...
           class="bg-white rounded-xl border border-slate-200 p-5 hover:border-brand-300 hover:shadow-sm transition-all group">
            ...
            <div class="flex items-center gap-4 text-xs text-slate-400">
                ...
                <span class="flex items-center gap-1">
                    ...
                    {{ $formation->template_count }} template(s)
                </span>
            </div>
        </a>
        @empty
        <div class="col-span-full bg-white rounded-xl border border-slate-200 p-8 text-center">
            <p class="text-sm text-slate-400">Aucune formation. Créez-en une d'abord depuis l'onglet Formations.</p>
        </div>
        @endforelse
    </div>
</div>
