{{-- File: resources/views/admin/flashcards/subchapter.blade.php --}}
@extends('layouts.app')
...
<div class="fade-in max-w-3xl">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                <form method="POST" action="{{ route('admin.flashcards.generate', $subchapter) }}">
                    @csrf
                    <button type="submit"
                            class="bg-amber-50 hover:bg-amber-100 border border-amber-200 text-amber-700 text-sm font-medium py-2 px-4 rounded-lg transition-colors inline-flex items-center gap-1.5"
                            onclick="this.disabled=true; this.innerHTML='Génération...'; this.form.submit();">
                        ⚡ Générer par IA
                    </button>
                </form>
                <a href="{{ route('admin.flashcards.study', ['sub_chapter_id' => $subchapter->id]) }}"
                   class="bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors inline-flex items-center gap-1.5">
                    Tester →
                </a>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 mb-6" x-data="{ open: false }">
        ...
        <form method="POST" action="{{ route('admin.flashcards.store') }}" x-show="open" x-transition x-cloak class="mt-4 space-y-3">
            @csrf
            ...
            <div class="flex items-center gap-3 flex-wrap">
                <label class="inline-flex items-center gap-2 text-xs text-slate-600">
                    ...
                    Template (distribué aux apprenants inscrits)
                </label>
            ...
            <div class="bg-white rounded-xl border border-brand-100 p-5" x-data="{ editing: false }">
                <div x-show="!editing" class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex items-center gap-1 flex-shrink-0">
                        <button type="button" @click="editing = true" class="text-slate-400 hover:text-slate-600 p-1" title="Modifier">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </button>
                        <form method="POST" action="{{ route('admin.flashcards.destroy', $card) }}" onsubmit="return confirm('Supprimer ce template ?')">
                            @csrf @method('DELETE')
                ...
                <form x-show="editing" x-transition x-cloak method="POST" action="{{ route('admin.flashcards.update', $card) }}" class="space-y-3">
                    @csrf @method('PUT')
                    <textarea name="question" rows="2" required class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500">{{ $card->question }}</textarea>
                    <textarea name="answer" rows="2" required class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500">{{ $card->answer }}</textarea>
                    <div class="flex gap-2">
                        ...
                        <button type="button" @click="editing = false" class="text-sm text-slate-500 hover:text-slate-700">Annuler</button>
                    </div>
                </form>
            </div>
            @endforeach
    ...
    {{-- Personal flashcards (admin's own study deck) --}}
    @if($personal->count() > 0)
    <div>
        <h2 class="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-slate-400"></span>
            Mes cartes personnelles ({{ $personal->count() }})
            <span class="text-xs font-normal text-slate-400">— pour tester</span>
        </h2>
        <div class="space-y-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-3 mt-2 text-xs text-slate-400 flex-wrap">
                            ...
                            @if($card->next_review_at)
                                <span>Prochaine : {{ $card->next_review_at->diffForHumans() }}</span>
                            @else
                                <span class="text-amber-500">Nouvelle</span>
                            @endif
                            @if($card->difficulty === 1 && $card->review_count >= 3)
                                <span class="text-emerald-600 font-medium">Maîtrisée ✓</span>
                            @endif
                        </div>
                    </div>
                    <div class="flex items-center gap-1 flex-shrink-0">
                        ...
                        <form method="POST" action="{{ route('admin.flashcards.destroy', $card) }}" onsubmit="return confirm('Supprimer cette carte personnelle ?')">
                            @csrf @method('DELETE')
                            <button type="submit" class="text-slate-400 hover:text-rose-500 p-1" title="Supprimer">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-8 text-center">
        <p class="text-sm text-slate-400 mb-2">Aucune flashcard pour ce sous-chapitre.</p>
