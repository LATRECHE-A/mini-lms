{{-- resources/views/student/flashcards/subchapter.blade.php --}}
@extends('layouts.app')
...
<div class="fade-in max-w-3xl">
    <div class="mb-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                @if($dueCount > 0)
                <a href="{{ route('student.flashcards.study', ['sub_chapter_id' => $subchapter->id]) }}" class="bg-brand-600 hover:bg-brand-700 text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors inline-flex items-center gap-1.5">
                    Étudier ({{ $dueCount }})
                </a>
                @endif
            </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-5 mb-3" x-data="{ editing: false }">
        ...
        <form x-show="editing" x-transition method="POST" action="{{ route('student.flashcards.update', $card) }}" class="space-y-3">
            @csrf @method('PUT')
