{{-- File: resources/views/student/subchapters/create.blade.php --}}
@extends('layouts.app')
...
<div class="fade-in max-w-2xl">
    <div class="mb-8">
        <a href="{{ route('student.formations.show', $formation) }}" class="text-sm text-slate-500 hover:text-slate-700 inline-flex items-center gap-1 mb-2">
            ...
            {{ $formation->name }} → {{ $chapter->title }}
        </a>
        <h1 class="text-2xl font-bold text-slate-900">Nouveau sous-chapitre</h1>
    </div>

    <div class="bg-white rounded-xl border border-slate-200 p-6">
        <form method="POST" action="{{ route('student.subchapters.store', $chapter) }}" class="space-y-5">
            @csrf
            <div>
            ...
            <div>
                <label for="content" class="block text-sm font-medium text-slate-700 mb-1.5">Contenu</label>
                <textarea id="content" name="content" rows="8"
                          class="w-full px-4 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">{{ old('content') }}</textarea>
                <p class="text-xs text-slate-400 mt-1">Vous pourrez ensuite éditer le contenu avec l'éditeur riche.</p>
            </div>
            <div>
                <label for="order" class="block text-sm font-medium text-slate-700 mb-1.5">Ordre</label>
                <input type="number" id="order" name="order" value="{{ old('order', $nextOrder ?? 1) }}" min="0"
