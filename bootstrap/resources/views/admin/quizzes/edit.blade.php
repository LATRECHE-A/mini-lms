...
<div class="fade-in max-w-2xl">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-slate-900">Modifier le quiz</h1>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-6">
        <form method="POST" action="{{ route('admin.quizzes.update', $quiz) }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title', $quiz->title) }}" required class="w-full px-4 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
            </div>
            <div>
                <label for="description" class="block text-sm font-medium text-slate-700 mb-1.5">Description</label>
                <textarea id="description" name="description" rows="2" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">{{ old('description', $quiz->description) }}</textarea>
            </div>
            <div>
                ...
                <select id="status" name="status" class="w-48 px-4 py-2.5 rounded-lg border border-slate-300 text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                    ...
                    <option value="published" {{ $quiz->status === 'published' ? 'selected' : '' }}>Publié</option>
                </select>
            </div>
