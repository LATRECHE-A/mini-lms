...
<div class="fade-in max-w-4xl">
    <div class="mb-8">
        <a href="{{ route('admin.ai.show', $generation) }}" class="text-sm text-slate-500 hover:text-slate-700 inline-flex items-center gap-1 mb-2">
        ...
        <h1 class="text-2xl font-bold text-slate-900">Modifier le contenu généré</h1>
    </div>

    @if(!$parsed['parsed'])
    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-800 mb-6 flex items-center gap-2">
        ...
        Ce contenu ne peut pas être modifié car il n'est pas au format structuré. Régénérez-le d'abord.
    </div>
    @else

    <form method="POST" action="{{ route('admin.ai.update', $generation) }}" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div class="flex-1">
                            <label class="block text-xs text-slate-500 mb-1">Titre du sous-chapitre</label>
                    ...
                    <div>
                        <label class="block text-xs text-slate-500 mb-1">Contenu (HTML simple autorisé : h3, p, ul, li, strong, em)</label>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <a href="{{ route('admin.ai.show', $generation) }}" class="text-sm text-slate-500 hover:text-slate-700">Annuler</a>
        </div>
    </form>
