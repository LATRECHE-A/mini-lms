...
<div class="fade-in max-w-3xl" x-data="aiPlayground()">
    <div class="mb-8">
        ...
        <p class="text-sm text-slate-500 mt-1">Décrivez le contenu d'étude que vous souhaitez générer.</p>
    </div>

    @if(!$isAvailable)
    <div class="bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-lg mb-6 text-sm">
        Service IA non disponible. Veuillez contacter un administrateur.
    </div>
    @endif

    <form method="POST" action="{{ route('student.ai.generate') }}" enctype="multipart/form-data"
        ...
        <div :class="loading ? 'opacity-50 pointer-events-none select-none' : ''" class="transition-opacity duration-300 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <div class="relative">
                    <textarea id="prompt" name="prompt" rows="5" required minlength="10" maxlength="5000" x-ref="promptArea"
                        ...
                        placeholder="Ex: Expliquez-moi les bases de la programmation orientée objet...">{{ old('prompt') }}</textarea>

                    <button type="button" :class="micAvailable ? '' : 'hidden'" @click.stop="toggleVoice()"
                ...
                @error('prompt')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
            </div>

            {{-- TYPE + CHAPTERS + DEPTH (identical labels as admin) --}}
            <div class="bg-white rounded-xl border border-slate-200 p-6">
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        @error('chapter_count')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        @error('depth')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
                    </div>
                </div>
            </div>
        ...
        <div class="mt-6">
            <button type="submit" :disabled="loading || !{{ $isAvailable ? 'true' : 'false' }}"
...
function aiPlayground() {
    return {
        chapterCount: {{ (int) old('chapter_count', 3) }},
        depth: '{{ old('depth', 'standard') }}',
        files: [], totalSize: 0, fileError: '', dragOver: false,
        micAvailable: false, micUnavailableReason: '', recognition: null, isListening: false,
        interimTranscript: '', voiceLang: 'fr-FR', voiceError: '', loading: false,

        init() {
            try { this.voiceLang = localStorage.getItem('voiceLang') || 'fr-FR'; } catch(e) {}
            const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
            const isSecure = location.protocol === 'https:' || location.hostname === 'localhost' || location.hostname === '127.0.0.1';
            if (!SR) { this.micUnavailableReason = 'Reconnaissance vocale non supportée (utilisez Chrome ou Edge).'; return; }
            if (!isSecure) { this.micUnavailableReason = 'La saisie vocale nécessite HTTPS. Accédez au site via https://.'; return; }
            this.micAvailable = true;
            const rec = new SR();
            rec.continuous = false; rec.interimResults = true; rec.maxAlternatives = 1; rec.lang = this.voiceLang;
            rec.onresult = (e) => {
                let interim = '', final_ = '';
