    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6">
                ...
                <figure class="mb-6">
                    <img src="{{ $subchapter->image_url }}" alt="{{ $subchapter->image_alt ?? $subchapter->title }}" loading="lazy"
                        class="w-full rounded-lg border border-slate-200 object-cover max-h-80">
                    @if($subchapter->image_credit)
                    <figcaption class="mt-2 text-xs text-slate-400 text-center italic">{{ $subchapter->image_credit }}</figcaption>
                    @endif
                </figure>
                @endif
                ...
                @if($subchapter->content)
                    <div class="prose prose-sm prose-slate max-w-none">
                        {!! \App\Services\ContentSanitizer::render($subchapter->content) !!}
                    </div>
                @else
        ...
        <div>
            <div class="bg-white rounded-xl border border-slate-200">
                ...
                <form method="POST" action="{{ route('student.notes.store') }}" class="px-5 py-3 border-b border-slate-100">
                    ...
                    <input type="text" name="title" placeholder="Titre de la note..." required class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm mb-2 focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                ...
                @empty
                <div class="px-5 py-4 text-center text-xs text-slate-400">Aucune note pour ce sous-chapitre.</div>
                @endforelse
            </div>
        </div>
    </div>
